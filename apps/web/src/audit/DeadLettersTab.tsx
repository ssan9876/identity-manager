...
export function DeadLettersTab() {
  ...
  useEffect(() => {
    ...
    fetchDeadLetters(accessToken, { limit: LIMIT, offset })
      .then(async (res) => {
        if (cancelled) return
        setPage(res)
  ...
  return (
    <div className="audit-log">
      <p className="audit-log__intro">
        Events that failed to sync to Keycloak after {DEFAULT_MAX_ATTEMPTS} attempts. These will{' '}
        <strong>not</strong> retry automatically — resolve the underlying issue, then run the reconciliation job
        (<span className="mono">pnpm reconcile</span>) to repair them.
      </p>
      ...
      <div className="table-wrap">
        {loadError !== null ? (
          ...
          <div className="empty-state">
            ...
            <p>Every sync event has landed — nothing is stuck or permanently failed right now.</p>
          </div>
        ) : (
