...
# The Content-Security-Policy both vhosts `include` (finding CS-M2). It is
# generated by the web build from the index.html just produced, because it
# carries that file's inline-script sha256 — see deploy/nginx/idm.conf. Assert
# it here so a missing one is named plainly rather than surfacing later as
# `nginx -t` failing on an include it cannot open.
[[ -f "$REPO_ROOT/apps/web/dist/csp.conf" ]] || die "web build produced no dist/csp.conf (CSP header snippet)"
